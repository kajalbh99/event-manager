 ...
 <div class="row">
    <div class="col-md-12">
        ...
        <div class="portlet box blue">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-edit"></i>Edit Transportation
                </div>
            </div>
            <div class="portlet-body">
                {{ Form::open(array('route'=>['transportation-edit',$transportation_details->id],'files'=>true,'id' => 'transportationForm', 'onsubmit'=>'return validate()' )) }}
                    <div class="row transportation">
                        <div class='col-sm-4'>                       
                            <div class="form-group">
                                {{ Form::label('transportation', 'Transportation Name',['class' => 'form-label']) }}
                                {{ Form::text('transportation_name',$transportation_details->transportation_name, ['class' => 'form-control','placeholder' => 'Transportation Name']) }}
                            </div>                                           
                            <div class="form-group">
                                {{ Form::label('transportation', 'Transportation Location',['class' => 'form-label']) }}
                                {{ Form::text('transportation_location',$transportation_details->transportation_location, ['class' => 'form-control','placeholder' => 'Transportation Location']) }}                    
                            </div>
                            <div class="form-group">
                                {{ Form::label('transportation', 'Transportation Slug',['class' => 'form-label']) }}
                                {{ Form::text('transportation_slug',$transportation_details->transportation_slug, ['class' => 'form-control','placeholder' => 'Transportation Slug']) }}
                            </div>
                        </div>
                        <div class="col-sm-4">
							<div class="form-group">
                                ...
                                <select class="form-control" name="carnival_id" id="carnival_id" data-parsley-required="true">
                                    ...
                                    {
                                        <option {{ $carnival->id == $transportation_details->carnival_id ? 'selected="selected"' : '' }}  value="{{ $carnival->id }}">{{ $carnival->carnival_name }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="country_id" id="country_id" data-parsley-required="true">
                                    ...
                                    {
                                        <option {{ $country->id == $transportation_details->country_id ? 'selected="selected"' : '' }} value="{{ $country->id }}">{{ $country->country_name }}</option>
                                    }
                                    @endforeach
                                </select>
                            </div>
							<div class="form-group state">
                                ...
                                <select class="form-control" name="state_id" id="state" data-parsley-required="true">
...

                                    {
                                        ...
                                        <option  value="{{ $state->id }}" @if($transportation_details->state_id==$state->id) selected @endif>{{ $state->state_name }}</option>
                                    ...
                                    <div class="form-group">
                                        <table>
                                            <tr><td name="image" id="image" align="center"></td>       
                                                @if(!empty($transportation_details->transportation_banner))
                                                    <img id="new" class="banner img-responsive" src="{{asset('public/uploads/transportation_banners/'.$transportation_details->id.'/'.$transportation_details->transportation_banner)}}">
                                                @else
                    ...
                    <div class="form-group">
                        ...
                        {{ Form::textarea('transportation_description',$transportation_details->transportation_description, ['size' => '30x5','class' => 'form-control','placeholder' => 'Transportation Description']) }}
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-4" style="display:none;">                           
                            <div class="form-group">
                                    ...
                                    {{ Form::checkbox('is_active', 1, ($transportation_details->is_active == "1"), ['class' => '','id'=>'active']) }}                      
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                {{ Form::radio('transportation_privacy', 'PUBLIC',($transportation_details->transportation_privacy == 'PUBLIC' ), array('id'=>'public','class'=>'radio-btn')) }}
                                {{ Form::label('private', 'Private') }}
                                {{ Form::radio('transportation_privacy', 'PRIVATE', ($transportation_details->transportation_privacy == 'PRIVATE' ), array('id'=>'private','class'=>'radio-btn')) }}                      
                            </div>
                        </div>
                    </div>
	...
	<div class="portlet box blue">
		...
		<div class="portlet-body">
			...
			<div class="row">
				@foreach($transportation_gallery as $images )
							...
							<div class="form-group">
								<div style="height: 190px;" class="form-body">
									<div class="form-group">
										...
										<img class="banner img-responsive" src="{{asset('public/uploads/transportation_gallery/'.$transportation_details->id.'/'.$image)}}">                                                                                                                         
									  </div>
								</div>
							</div>
						</div>
...
</div>	<div class="col-md-12">        <!-- BEGIN EXAMPLE TABLE PORTLET-->        <div class="portlet box blue">            <div class="portlet-title">                <div class="caption">                    <i class="fa fa-edit"></i>Reviews                </div>            </div>            <div class="portlet-body">  				@if(count($reviews)>0)                    <table class="table table-striped table-hover table-bordered" id="transportationTable">						<thead>							<tr>								<th>									Sr. no.								</th>								<th>									User								</th>								<th>									 Review								</th>								<th>									rating								</th>															<th>									Action								</th>							</tr>						</thead>						<tbody>													@foreach($reviews as $key => $review)							<tr>								<td>									{{ $key+1 }}								</td>								<td>									{{$review->user->email}}								</td>								<td>									{{ $review->review_description }}								</td>								<td>									{{$review->rating}}								</td>								<td> 									@if($review->is_approved)											<button class="btn btn-danger" onclick = "return disapprove('{{$review->id}}');" type="button">Disapprove</button>									@else										<button class="btn btn-success test" onclick="return approve('{{$review->id}}');" type="button">Approve</button>																		@endif	&nbsp;<a class="btn btn-primary" href="{{route('view-transportation-review',[$review->id])}}">View</a>							</td>                        							</tr>							@endforeach																		</tbody>					</table>                                            				@else					<h4>No Reviews found.</h4>				@endif            </div>        </div>        <!-- END EXAMPLE TABLE PORTLET-->
<button type="submit" class="btn btn-primary">Submit</button>
@if($transportation_details->is_active=='0')
<a href="{{route('transportation_change_approve_status',[$transportation_details->id,1])}}" class="btn btn-success">Activate</a>
@else
<a href="{{route('transportation_change_approve_status',[$transportation_details->id,0])}}" class="btn btn-danger">Deactivate</a>	
@endif
{{ Form::close() }}
 ...
 <script>	
    ...
    });		function disapprove(value){		if(confirm("Are you sure you want to Disapprove this?")){			$.ajax({				type: "post",				url: "{{ route('transportation-review-disapprove') }}",				data: {'_token':'{{ csrf_token() }}', 'id':value},				success: function(result){					location.reload();				}			});		}		else{			return false;		}					}		function approve(value){				if(confirm("Are you sure you want to Approve this?")){			$.ajax({				type: "post",				url: "{{ route('transportation-review-approve') }}",				data: {'_token':'{{ csrf_token() }}', 'id':value},				success: function(result){					location.reload();				}			});		}		else{			return false;		}	}
    ...
    $( "#transportationForm" ).validate( {
        ...
        messages: {
            transportation_name: "Please enter transportation name",
            transportation_location: "Please enter location",
            transportation_slug: "Please enter slug name",
            country_id: "Please select country",
            carnival_id: "Please  select carnival",
            transportation_description: "Please enter transportation description",
            transportation_type: "Please select transportation type",
            one_time_transportation_start_date: "Please select transportation date",
